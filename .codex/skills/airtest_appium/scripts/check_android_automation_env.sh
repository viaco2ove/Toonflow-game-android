...
ANDROID_HOME_FALLBACK="${ANDROID_HOME:-$HOME/.local/android-sdk}"
ANDROID_SDK_ROOT_FALLBACK="${ANDROID_SDK_ROOT:-$ANDROID_HOME_FALLBACK}"
JAVA_HOME_FALLBACK="${JAVA_HOME:-$HOME/.local/opt/jdk-17}"

if [ -d "$JAVA_HOME_FALLBACK" ]; then
  export JAVA_HOME="$JAVA_HOME_FALLBACK"
  export PATH="$JAVA_HOME/bin:$PATH"
fi

if [ -d "$ANDROID_HOME_FALLBACK" ]; then
  export ANDROID_HOME="$ANDROID_HOME_FALLBACK"
  export ANDROID_SDK_ROOT="$ANDROID_SDK_ROOT_FALLBACK"
